...
import com.pidev.Service.ServiceReaction;
...
@Override
    protected void updateItem(Publication pub, boolean empty) {
        ...
        if(empty || pub == null) {
         ...
         desription.setText(pub.getcontenu());
         contenuee.setText(pub.gettitre());
        ...
        /* String path=this.getItem().getcontenu();          
            ...
            List<Medias> listemedia=new ArrayList<Medias>();
            for(Medias m :lm){   
            System.out.println("listemedia");
            if(this.getItem().getId()==this.getItem().getId()){
            listemedia.add(m) ;}
          
            mediacontainer.setCellFactory(mf);
            
            mediacontainer.getItems().clear();
            mediacontainer.getItems().addAll(listemedia);
                                         
            }
         if(this.getItem().getId_auteur()!=Userconnected.getId())
         {
    ...
    InputStream im =getClass().getResourceAsStream(Userconnected.getImguser());
    Image imageuser1 = new Image(im);
    imageuser.setImage(imageuser1);
    nameuser.setText(Userconnected.getNom());
        }
        setGraphic(pubbody);
   }
    ...
    private void transferedata() {
      ...
      String d= this.getItem().gettitre();
